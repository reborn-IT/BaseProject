...
function SearchableDropDown({
  ...
  const inputHandler = (e) => {
    ...
    if (createModeStatus) {
      setList(
        (oldList) => [...oldList, { id: oldList[oldList.length].id + 1, title: CREATE_NEW_TITLE }],
      );
    }
  };
  ...
  return (
    ...
    >
      <svg
        className={`h-6 w-6 absolute top-4 transform transition-all duration-150 right-6 z-10 ${ListOpened ? 'rotate-0' : 'rotate-180'}`}
        fill="none"
        viewBox="0 0 24 24"
        stroke="currentColor"
        strokeWidth="2"
      ...
      >
        {
                  List.map(({ id, title }) => (
                    <button
                      ...
                      className={`capitalize px-4 py-3 transition-all duration-100 ease-linear text-left ${extraTailwind?.item} ${
